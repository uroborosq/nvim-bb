...
- Neovim integration:
  ...
  - Suggestion acceptance now also works from diff buffers when the comment anchor path and current buffer path match after normalization (including `a/`, `b/`, relative paths, and diffview URIs). Use `:BBPRAcceptSuggestion` (or `<leader>rS`), configurable via `setup({ accept_suggestion_map = "..." })`.
